...
describe('POST /api/extension/outlook/disconnect', () => {
  beforeEach(() => {
  ...
  it('requires an authenticated session before disconnecting', async () => {
    ...
    const response = await POST(new Request('https://calendarvlu.test/api/extension/outlook/disconnect', { method: 'POST', headers: { origin: 'https://calendarvlu.test' } }))

    expect(response.status).toBe(401)
    await expect(response.json()).resolves.toMatchObject({ error: { code: 'OUTLOOK_NOT_CONNECTED' } })
    expect(prisma.account.updateMany).not.toHaveBeenCalled()
  })

  it('nullifies outlook tokens and preserves session', async () => {
    ...
    expect(prisma.account.updateMany).toHaveBeenCalledWith({
      where: { userId: 'user-1', provider: 'microsoft-entra-id' },
