...
export async function GET(req: NextRequest) {
    ...
    let urlLichFetch = ''
    if (lichType == 'lichHoc') 
        urlLichFetch = 'DrawingStudentSchedule_Perior'
    else if (lichType == 'lichThi')
        urlLichFetch = 'DrawingStudentExamSchedule_Perior'
    else 
        return new Response('Invalid lichType', { status: 400 });

    const vluCalendar = await fetch(
        `${vluHomeUrl}/${urlLichFetch}?YearStudy=${yearStudy}&TermID=${termID}`,
        {
          method: 'GET',
