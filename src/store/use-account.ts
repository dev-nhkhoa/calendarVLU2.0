...
export const useAccountStore = create<useAccountStoreProps>((set) => {
  return {
    ...
    addAccount: (account: Account, userEmail: string) => {
      try {
        fetch('/api/accounts', { method: 'POST', body: JSON.stringify({ account: account, userEmail: userEmail }) }).then((res) => {
    ...
    deleteAccount: (account: Account) => {
      ...
      try {
        fetch('/api/accounts', { method: 'DELETE', body: JSON.stringify({ provider: 'vanLang' }) }).then((res) => {
          if (!res.ok) throw new Error('Server Error!')
